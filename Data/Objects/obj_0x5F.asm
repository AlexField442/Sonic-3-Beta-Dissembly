;===============================================================================
; Objeto 0x5F - Mola retrátil usada no modo 2 jogadores
; ->>>  
;===============================================================================
Offset_0x019FD6:
                dc.b    $0C, $08, $00, $01, $0C, $08, $00, $00
                dc.b    $0C, $08, $00, $01, $08, $0C, $01, $00
                dc.b    $0C, $08, $02, $01, $0C, $08, $03, $01
                dc.b    $0C, $08, $02, $01, $0C, $08, $03, $01                  
;-------------------------------------------------------------------------------
Obj_0x5F_Retracting_Spring:                                    ; Offset_0x019FF6
                jsr     (AllocateObjectAfterCurrent)                  ; Offset_0x011DE0
                bne     Offset_0x01A0CC
                move.l  #Offset_0x01A38E, (A1)
                move.l  #Retracting_Spring_Mappings, mappings(A1) ; Offset_0x01A408, $000C
                move.w  #$0391, art_tile(A1)                         ; $000A
                cmpi.b  #EMz_Id, (Current_Zone).w                   ; $12, $FFFFFE10
                bne.s   Offset_0x01A024
                move.l  #EMz_Retracting_Spring_Mappings, mappings(A1) ; Offset_0x01A410, $000C
Offset_0x01A024:
                move.b  subtype(A0), D0                              ; $002C
                move.b  D0, subtype(A1)                              ; $002C
                lsr.b   #$02, D0
                andi.w  #$001C, D0
                lea     Offset_0x019FD6(PC, D0), A2
                move.b  (A2)+, width_pixels(A1)                             ; $0007
                move.b  (A2)+, height_pixels(A1)                            ; $0006
                move.b  (A2)+, D0
                move.b  D0, mapping_frame(A1)                               ; $0022
                bne.s   Offset_0x01A04C
                move.l  #Offset_0x01A3E8, (A1)
Offset_0x01A04C:
                andi.b  #$F0, subtype(A0)                            ; $002C
                move.b  (A2)+, D1
                or.b    D1, subtype(A0)                              ; $002C
                move.w  x_pos(A0), x_pos(A1)                      ; $0010, $0010
                move.w  y_pos(A0), y_pos(A1)                      ; $0014, $0014
                moveq   #$00, D2
                moveq   #$00, D3
                cmpi.b  #$01, D0
                bne.s   Offset_0x01A07E
                moveq   #$0C, D2
                btst    #$00, status(A0)                             ; $002A
                bne.s   Offset_0x01A07A
                neg.w   D2
Offset_0x01A07A:
                add.w   D2, x_pos(A1)                                    ; $0010
Offset_0x01A07E:
                cmpi.b  #$02, D0
                bne.s   Offset_0x01A08A
                moveq   #$0B, D3
                add.w   D3, y_pos(A1)                                    ; $0014
Offset_0x01A08A:
                cmpi.b  #$03, D0
                bne.s   Offset_0x01A096
                moveq   #-$0B, D3
                add.w   D3, y_pos(A1)                                    ; $0014
Offset_0x01A096:
                move.w  D2, objoff_30(A1)                       ; $0030
                move.w  D3, objoff_32(A1)                       ; $0032
                bclr    #$01, status(A0)                             ; $002A
                beq.s   Offset_0x01A0AC
                move.b  #$01, objoff_3C(A0)                     ; $003C
Offset_0x01A0AC:
                move.b  status(A0), status(A1)            ; $002A, $002A
                move.b  render_flags(A0), render_flags(A1)              ; $0004, $0004
                ori.b   #$04, render_flags(A1)                              ; $0004
                move.w  #$0200, priority(A1)                         ; $0008
                move.w  A0, objoff_3E(A1)                       ; $003E
                move.w  A1, objoff_3E(A0)                       ; $003E
Offset_0x01A0CC:
                bsr     Obj_0x07_Springs_2P                    ; Offset_0x019268
Offset_0x01A0D0:                
                move.b  subtype(A0), D0                              ; $002C
                lsr.w   #$03, D0
                andi.w  #$000E, D0
                move.w  Offset_0x01A0E2(PC, D0), D1
                jmp     Offset_0x01A0E2(PC, D1)   
;-------------------------------------------------------------------------------
Offset_0x01A0E2:
                dc.w    Offset_0x01A0F2-Offset_0x01A0E2
                dc.w    Offset_0x01A10E-Offset_0x01A0E2
                dc.w    Offset_0x01A12A-Offset_0x01A0E2
                dc.w    Offset_0x01A146-Offset_0x01A0E2
                dc.w    Offset_0x01A162-Offset_0x01A0E2
                dc.w    Offset_0x01A17E-Offset_0x01A0E2
                dc.w    Offset_0x01A162-Offset_0x01A0E2
                dc.w    Offset_0x01A17E-Offset_0x01A0E2  
;-------------------------------------------------------------------------------
Offset_0x01A0F2:
                move.l  #Offset_0x01A0F8, (A0)
Offset_0x01A0F8:                
                bsr     Offset_0x01A19A
                moveq   #$00, D0
                move.b  objoff_36(A0), D0                       ; $0036
                add.w   objoff_34(A0), D0                       ; $0034
                move.w  D0, y_pos(A0)                                    ; $0014
                bra     Offset_0x01943A 
;-------------------------------------------------------------------------------
Offset_0x01A10E:
                move.l  #Offset_0x01A114, (A0)
Offset_0x01A114:                
                bsr     Offset_0x01A19A
                moveq   #$00, D0
                move.b  objoff_36(A0), D0                       ; $0036
                add.w   objoff_32(A0), D0                       ; $0032
                move.w  D0, x_pos(A0)                                    ; $0010
                bra     Offset_0x0195C2   
;-------------------------------------------------------------------------------
Offset_0x01A12A:
                move.l  #Offset_0x01A130, (A0)
Offset_0x01A130:                
                bsr     Offset_0x01A19A
                moveq   #$00, D0
                move.b  objoff_36(A0), D0                       ; $0036
                add.w   objoff_34(A0), D0                       ; $0034
                move.w  D0, y_pos(A0)                                    ; $0014
                bra     Offset_0x019852   
;-------------------------------------------------------------------------------
Offset_0x01A146:
                move.l  #Offset_0x01A14C, (A0)
Offset_0x01A14C:                
                bsr     Offset_0x01A19A
                moveq   #$00, D0
                move.b  objoff_36(A0), D0                       ; $0036
                add.w   objoff_34(A0), D0                       ; $0034
                move.w  D0, y_pos(A0)                                    ; $0014
                bra     Offset_0x0195C2    
;-------------------------------------------------------------------------------
Offset_0x01A162:
                move.l  #Offset_0x01A168, (A0)
Offset_0x01A168:                
                bsr     Offset_0x01A19A
                moveq   #$00, D0
                move.b  objoff_36(A0), D0                       ; $0036
                add.w   objoff_32(A0), D0                       ; $0032
                move.w  D0, x_pos(A0)                                    ; $0010
                bra     Offset_0x01943A  
;-------------------------------------------------------------------------------
Offset_0x01A17E:
                move.l  #Offset_0x01A184, (A0)
Offset_0x01A184:                
                bsr     Offset_0x01A19A
                moveq   #$00, D0
                move.b  objoff_36(A0), D0                       ; $0036
                add.w   objoff_32(A0), D0                       ; $0032
                move.w  D0, x_pos(A0)                                    ; $0010
                bra     Offset_0x019852
Offset_0x01A19A:
                tst.b   subtype(A0)                                  ; $002C
                bmi.s   Offset_0x01A200
                tst.w   objoff_3A(A0)                           ; $003A
                beq.s   Offset_0x01A1BC
                subq.w  #$01, objoff_3A(A0)                     ; $003A
                bne.s   Offset_0x01A1FE
                tst.b   render_flags(A0)                                    ; $0004
                bpl.s   Offset_0x01A1FE
                moveq   #Spike_Move_Sfx, D0                                ; $58
                jsr     (PlaySound)                           ; Offset_0x001176
                bra.s   Offset_0x01A1FE
Offset_0x01A1BC:
                tst.w   objoff_38(A0)                           ; $0038
                beq.s   Offset_0x01A1DE
                subi.w  #$0800, objoff_36(A0)                   ; $0036
                bcc.s   Offset_0x01A1FE
                move.w  #$0000, objoff_36(A0)                   ; $0036
                move.w  #$0000, objoff_38(A0)                   ; $0038
                move.w  #$003C, objoff_3A(A0)                   ; $003A
                bra.s   Offset_0x01A1FE
Offset_0x01A1DE:
                addi.w  #$0800, objoff_36(A0)                   ; $0036
                cmpi.w  #$1800, objoff_36(A0)                   ; $0036
                bcs.s   Offset_0x01A1FE
                move.w  #$1800, objoff_36(A0)                   ; $0036
                move.w  #$0001, objoff_38(A0)                   ; $0038
                move.w  #$003C, objoff_3A(A0)                   ; $003A
Offset_0x01A1FE:
                rts
Offset_0x01A200:
                tst.b   objoff_3C(A0)                           ; $003C
                beq     Offset_0x01A2CA
                tst.w   objoff_3A(A0)                           ; $003A
                beq.s   Offset_0x01A220
                subq.w  #$01, objoff_3A(A0)                     ; $003A
                bne     Offset_0x01A2C8
                move.w  #$0000, objoff_38(A0)                   ; $0038
                bra     Offset_0x01A2C8
Offset_0x01A220:
                tst.w   objoff_38(A0)                           ; $0038
                bne     Offset_0x01A2AE
                tst.w   objoff_36(A0)                           ; $0036
                beq     Offset_0x01A2C8
                subi.w  #$0800, objoff_36(A0)                   ; $0036
                bhi     Offset_0x01A2C8
                move.w  #$0000, objoff_36(A0)                   ; $0036
                move.b  subtype(A0), D0                              ; $002C
                andi.b  #$F0, D0
                cmpi.b  #$E0, D0
                bne.s   Offset_0x01A27A
                addi.w  #$0016, y_pos(A0)                                ; $0014
                bchg    #01, status(A0)                              ; $002A
                bchg    #01, render_flags(A0)                               ; $0004
                andi.b  #$0F, subtype(A0)                            ; $002C
                ori.b   #$F0, subtype(A0)                            ; $002C
                move.l  #Offset_0x01A0D0, (A0)
                move.w  objoff_3E(A0), A1                       ; $003E
                neg.w   objoff_32(A1)                           ; $0032
Offset_0x01A27A:
                cmpi.b  #$F0, D0
                bne.s   Offset_0x01A2AC
                subi.w  #$0016, y_pos(A0)                                ; $0014
                bchg    #01, status(A0)                              ; $002A
                bchg    #01, render_flags(A0)                               ; $0004
                andi.b  #$0F, subtype(A0)                            ; $002C
                ori.b   #$E0, subtype(A0)                            ; $002C
                move.l  #Offset_0x01A0D0, (A0)
                move.w  objoff_3E(A0), A1                       ; $003E
                neg.w   objoff_32(A1)                           ; $0032
Offset_0x01A2AC:
                bra.s   Offset_0x01A2C8
Offset_0x01A2AE:
                addi.w  #$0800, objoff_36(A0)                   ; $0036
                cmpi.w  #$1800, objoff_36(A0)                   ; $0036
                bcs.s   Offset_0x01A2C8
                move.w  #$1800, objoff_36(A0)                   ; $0036
                move.w  #$00B4, objoff_3A(A0)                   ; $003A
Offset_0x01A2C8:
                rts
Offset_0x01A2CA:
                tst.w   objoff_3A(A0)                           ; $003A
                beq.s   Offset_0x01A2E2
                subq.w  #$01, objoff_3A(A0)                     ; $003A
                bne     Offset_0x01A38C
                move.w  #$0000, objoff_38(A0)                   ; $0038
                bra     Offset_0x01A38C
Offset_0x01A2E2:
                tst.w   objoff_38(A0)                           ; $0038
                beq.s   Offset_0x01A302
                subi.w  #$0800, objoff_36(A0)                   ; $0036
                bcc     Offset_0x01A38C
                move.w  #$0000, objoff_36(A0)                   ; $0036
                move.w  #$00B4, objoff_3A(A0)                   ; $003A
                bra     Offset_0x01A38C
Offset_0x01A302:
                cmpi.w  #$1800, objoff_36(A0)                   ; $0036
                beq     Offset_0x01A38C
                addi.w  #$0800, objoff_36(A0)                   ; $0036
                cmpi.w  #$1800, objoff_36(A0)                   ; $0036
                bcs.s   Offset_0x01A38C
                move.w  #$1800, objoff_36(A0)                   ; $0036
                move.b  subtype(A0), D0                              ; $002C
                andi.b  #$F0, D0
                cmpi.b  #$E0, D0
                bne.s   Offset_0x01A35A
                addi.w  #$0016, y_pos(A0)                                ; $0014
                bchg    #01, status(A0)                              ; $002A
                bchg    #01, render_flags(A0)                               ; $0004
                andi.b  #$0F, subtype(A0)                            ; $002C
                ori.b   #$F0, subtype(A0)                            ; $002C
                move.l  #Offset_0x01A0D0, (A0)
                move.w  objoff_3E(A0), A1                       ; $003E
                neg.w   objoff_32(A1)                           ; $0032
Offset_0x01A35A:
                cmpi.b  #$F0, D0
                bne.s   Offset_0x01A38C
                subi.w  #$0016, y_pos(A0)                                ; $0014
                bchg    #01, status(A0)                              ; $002A
                bchg    #01, render_flags(A0)                               ; $0004
                andi.b  #$0F, subtype(A0)                            ; $002C
                ori.b   #$E0, subtype(A0)                            ; $002C
                move.l  #Offset_0x01A0D0, (A0)
                move.w  objoff_3E(A0), A1                       ; $003E
                neg.w   objoff_32(A1)                           ; $0032
Offset_0x01A38C:
                rts  
;-------------------------------------------------------------------------------
Offset_0x01A38E:
                move.w  objoff_3E(A0), A1                       ; $003E
                move.w  x_pos(A1), D2                                    ; $0010
                move.w  y_pos(A1), D3                                    ; $0014
                add.w   objoff_30(A0), D2                       ; $0030
                add.w   objoff_32(A0), D3                       ; $0032
                move.w  D2, x_pos(A0)                                    ; $0010
                move.w  D3, y_pos(A0)                                    ; $0014
                move.b  subtype(A0), D1                              ; $002C
                bpl.s   Offset_0x01A3C4
                andi.w  #$000F, D1
                lea     (Level_Trigger_Array).w, A3                  ; $FFFFF7E0
                tst.b   $00(A3, D1)
                beq.s   Offset_0x01A3C4
                move.w  #$0001, objoff_38(A1)                   ; $0038
Offset_0x01A3C4:
                moveq   #$00, D1
                move.b  width_pixels(A0), D1                                ; $0007
                addi.w  #$0007, D1
                moveq   #$00, D2
                move.b  height_pixels(A0), D2                               ; $0006
                move.w  D2, D3
                addq.w  #$01, D3
                move.w  x_pos(A0), D4                                    ; $0010
                jsr     (Solid_Object)                         ; Offset_0x013556
                jmp     (DisplaySprite)                        ; Offset_0x011148   
;-------------------------------------------------------------------------------
Offset_0x01A3E8:
                move.b  subtype(A0), D1                              ; $002C
                bpl.s   Offset_0x01A406
                andi.w  #$000F, D1
                lea     (Level_Trigger_Array).w, A3                  ; $FFFFF7E0
                tst.b   $00(A3, D1)
                beq.s   Offset_0x01A406
                move.w  objoff_3E(A0), A1                       ; $003E
                move.w  #$0001, objoff_38(A1)                   ; $0038
Offset_0x01A406:
                rts            
;------------------------------------------------------------------------------- 
Retracting_Spring_Mappings:                                    ; Offset_0x01A408
                dc.w    Offset_0x01A418-Retracting_Spring_Mappings
                dc.w    Offset_0x01A41A-Retracting_Spring_Mappings
                dc.w    Offset_0x01A422-Retracting_Spring_Mappings
                dc.w    Offset_0x01A422-Retracting_Spring_Mappings           
;-------------------------------------------------------------------------------  
EMz_Retracting_Spring_Mappings:                                ; Offset_0x01A410
                dc.w    Offset_0x01A418-EMz_Retracting_Spring_Mappings
                dc.w    Offset_0x01A42A-EMz_Retracting_Spring_Mappings
                dc.w    Offset_0x01A422-EMz_Retracting_Spring_Mappings
                dc.w    Offset_0x01A422-EMz_Retracting_Spring_Mappings
Offset_0x01A418:
                dc.w    $0000
Offset_0x01A41A:
                dc.w    $0001
                dc.w    $F406, $082F, $FFF8                
Offset_0x01A422:
                dc.w    $0001
                dc.w    $F809, $600D, $FFF4
Offset_0x01A42A:
                dc.w    $0001
                dc.w    $F40A, $4774, $FFF0                                                                                                                                                             
;===============================================================================
; Objeto 0x5F - Mola retrátil usada no modo 2 jogadores
; <<<-  
;===============================================================================  