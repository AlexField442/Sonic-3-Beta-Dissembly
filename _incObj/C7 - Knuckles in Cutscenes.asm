; ===========================================================================
; ---------------------------------------------------------------------------
; Object C7 - Knuckles in Cutscenes
; ---------------------------------------------------------------------------
; Offset_0x034BAA: Obj_0xC7_Knuckles:
ObjC7_CutsceneKnuckles:
		move.w	a0,(Obj_Knuckles_Mem_Address).w
		moveq	#0,d0
		move.b	subtype(a0),d0
		move.l	CutKnux_Index(pc,d0.w),a1
		move.l	a1,(a0)
		jmp	(a1)
; ===========================================================================
; Offset_0x034BBC:
CutKnux_Index:	dc.l	CutKnux_AIZ1
		dc.l	CutKnux_AIZ2
		dc.l	Knuckles_In_Carnival_Night_2_Lamp
		dc.l	Knuckles_In_Carnival_Night_2_Boss
		dc.l	Knuckles_In_Launch_Base_1
		dc.l	Knuckles_In_Launch_Base_2

; ===========================================================================
; ---------------------------------------------------------------------------
; Knuckles in Angel Island Act 1 (for the Final Intro)
; ---------------------------------------------------------------------------
; Offset_0x034BD4: Knuckles_In_Angel_Island_2:
CutKnux_AIZ1:
		moveq	#0,d0
		move.b	routine(a0),d0
		move.w	CutKnux_AIZ1_Index(pc,d0.w),d1
		jsr	CutKnux_AIZ1_Index(pc,d1.w)
		bsr.w	Offset_0x035538
		lea	Knuckles_PLC_Data(pc),a2
		jsr	(LoadDynamicPLC).l
		jmp	(DisplaySprite).l
; ===========================================================================
; Offset_0x034BF6:
CutKnux_AIZ1_Index:
		dc.w CutKnux_AIZ1_Init-CutKnux_AIZ1_Index
		dc.w CutKnux_AIZ1_Null-CutKnux_AIZ1_Index
; ===========================================================================
; Offset_0x034BFA:
CutKnux_AIZ1_Init:
		lea	Knuckles_Setup_Data(pc),a1
		jsr	(SetupSlottedObjectAttributes).l
		move.b	#8,mapping_frame(a0)
		move.w	#$1400,x_pos(a0)
		move.w	#$430,y_pos(a0)
		moveq	#Volume_Down,d0
		jsr	(PlaySound).l
		move.w	#$77,default_y_radius(a0)
		rts
; Offset_0x034C26:
CutKnux_AIZ1_Null:
		rts
; ---------------------------------------------------------------------------
; This code is disabled for a good reason, as re-enabling it would cause the
; distant palmtrees in the intro appear wrong. POSSIBLY indicative of an earlier
; build that lacked them, as the early palette tables include an intro palette
; that similarly doesn't work well with the final graphics.
; Offset_0x034C28: CutKnux_AIZ1_LoadPal:
		lea	Knuckles_Palette(pc),a1
		jmp	(Pal_Load_Line_1).l

; ===========================================================================
; ---------------------------------------------------------------------------
; Knuckles in Hydrocity Act 2
; ---------------------------------------------------------------------------
; Offset_0x034C32: Knuckles_In_Hydrocity_2:
CutKnux_AIZ2:
		moveq	#0,d0
		move.b	routine(a0),d0
		move.w	CutKnux_AIZ2_Index(pc,d0.w),d1
		jsr	CutKnux_AIZ2_Index(pc,d1.w)
		lea	Knuckles_PLC_Data(pc),a2
		jsr	(LoadDynamicPLC).l
		jmp	(Check_Delete_Touch_Slotted).l
; ===========================================================================
; Offset_0x034C50:
CutKnux_AIZ2_Index:
		dc.w CutKnux_AIZ2_Init-CutKnux_AIZ2_Index
		dc.w CutKnux_Wait-CutKnux_AIZ2_Index
		dc.w Offset_0x034CAE-CutKnux_AIZ2_Index
		dc.w Offset_0x034CFA-CutKnux_AIZ2_Index
		dc.w Offset_0x034D36-CutKnux_AIZ2_Index
		dc.w Offset_0x034D70-CutKnux_AIZ2_Index
; ===========================================================================
; Offset_0x034C5C:
CutKnux_AIZ2_Init:
		lea	Knuckles_Setup_Data(pc),a1
		jsr	(SetupSlottedObjectAttributes).l
		move.w	#$4B8E,x_pos(a0)
		move.w	#$17D,y_pos(a0)
		bset	#0,render_flags(a0)
		moveq	#Volume_Down,d0
		jsr	(PlaySound).l
		move.w	#$77,objoff_2E(a0)
		move.l	#CutKnux_AIZ2_PlayMusic,child(a0)
		lea	Knuckles_Palette(pc),a1
		jmp	(Pal_Load_Line_1).l
; ===========================================================================
; Offset_0x034C98:
CutKnux_Wait:
		jmp	(Run_Object_Wait_Timer_A0).l
; ---------------------------------------------------------------------------
; Offset_0x034C9E:
CutKnux_AIZ2_PlayMusic:
		move.b	#4,routine(a0)
		moveq	#Knuckles_Theme_Snd,d0
		jsr	(PlaySound).l
		rts

;-------------------------------------------------------------------------------
Offset_0x034CAE:
		move.w  x_pos(A0), D0				    ; $0010
		subq.w  #$02, D0
		cmpi.w  #$4B3C, D0
		bcs.s   Offset_0x034CC8
		move.w  D0, x_pos(A0)				    ; $0010
		lea     Offset_0x0355B1(PC), A1
		jmp     (Animate_Raw_A1)		       ; Offset_0x042092
Offset_0x034CC8:
		move.b  #$06, routine(A0)		            ; $0005
		bclr    #$00, render_flags(A0)		              ; $0004
		move.w  #$005F, objoff_2E(A0)		            ; $002E
		move.l  #Offset_0x034D06, child(A0)		  ; $0034
Offset_0x034CE2:		
		move.l  #Offset_0x0355C5, child_data(A0)             ; $0030
		clr.b   anim_frame_duration(A0)				 ; $0024
		clr.b   anim_frame(A0)				; $0023
		move.b  #$1C, mapping_frame(A0)		             ; $0022
Offset_0x034CF8:		
		rts
Offset_0x034CFA:
		jsr     (AnimateRaw)		          ; Offset_0x04208E
		jmp     (Run_Object_Wait_Timer_A0)             ; Offset_0x0423D2
;-------------------------------------------------------------------------------    
Offset_0x034D06:
		move.b  #$08, routine(A0)		            ; $0005
Offset_0x034D0C:		
		move.w  #$FF00, x_vel(A0)		          ; $0018
		move.w  #$FC00, y_vel(A0)		          ; $001A
Offset_0x034D18:		
		move.l  #Offset_0x0355BB, child_data(A0)             ; $0030
		clr.b   anim_frame_duration(A0)				 ; $0024
		clr.b   anim_frame(A0)				; $0023
		move.b  #$08, mapping_frame(A0)		             ; $0022
		move.b  #$13, y_radius(A0)		           ; $001E
		rts 
;-------------------------------------------------------------------------------  
Offset_0x034D36:
		jsr     (AnimateRaw)		          ; Offset_0x04208E
		jsr     (ObjectFall)		           ; Offset_0x0110FE
		tst.w   y_vel(A0)				  ; $001A
		bmi.s   Offset_0x034D62
		jsr     (ObjHitFloor)		          ; Offset_0x009D84
		tst.w   D1
		bpl.s   Offset_0x034D62
		bset    #$02, objoff_38(A0)		     ; $0038
		bne.s   Offset_0x034D64
		neg.w   x_vel(A0)				  ; $0018
		neg.w   y_vel(A0)				  ; $001A
Offset_0x034D62:
		rts
Offset_0x034D64:
		addq.b  #$02, routine(A0)		            ; $0005
		add.w   D1, y_pos(A0)				    ; $0014
		bra     Offset_0x034CE2            
;-------------------------------------------------------------------------------  
Offset_0x034D70:
		jmp     (AnimateRaw)		          ; Offset_0x04208E 
;------------------------------------------------------------------------------- 
Knuckles_In_Carnival_Night_2_Lamp:		             ; Offset_0x034D76
		lea     Offset_0x034DAE(PC), A1
		jsr     (Check_Camera_In_Range)		; Offset_0x043392
		moveq   #$00, D0
		move.b  routine(A0), D0		              ; $0005
		move.w  Offset_0x034DA2(PC, D0), D1
		jsr     Offset_0x034DA2(PC, D1)
		bsr     Offset_0x035538
		lea     Knuckles_PLC_Data(PC), A2              ; Offset_0x0355A4
		jsr     (LoadDynamicPLC)		  ; Offset_0x042A0A
		jmp     (Check_Delete_Touch_Slotted)           ; Offset_0x042C1E   
;-------------------------------------------------------------------------------
Offset_0x034DA2:
		dc.w    Offset_0x034DB6-Offset_0x034DA2
		dc.w    Offset_0x034DE2-Offset_0x034DA2
		dc.w    Offset_0x034E2E-Offset_0x034DA2
		dc.w    CutKnux_Wait-Offset_0x034DA2
		dc.w    Offset_0x034D36-Offset_0x034DA2
		dc.w    Offset_0x034E6A-Offset_0x034DA2 
;-------------------------------------------------------------------------------               
Offset_0x034DAE:
		dc.w    $0540, $0600, $3900, $3940		   
;-------------------------------------------------------------------------------
Offset_0x034DB6:
		lea     Knuckles_Setup_Data(PC), A1            ; Offset_0x035548
		jsr     (SetupSlottedObjectAttributes)              ; Offset_0x04298C
		move.w  #$0078, default_y_radius(A0)		         ; $0044
		moveq   #Volume_Down, D0				  ; -$20
		jsr     (PlaySound)		           ; Offset_0x001176
		move.w  (Sonic_Level_Limits_Min_Y).w, (Target_Camera_Min_Y).w ; $FFFFEE18, $FFFFFA96
		move.w  (Sonic_Level_Limits_Max_X).w, (Target_Camera_Max_X).w ; $FFFFEE16, $FFFFFA92
		move.w  #$3940, (Sonic_Level_Limits_Max_X).w         ; $FFFFEE16
		rts    
;-------------------------------------------------------------------------------
Offset_0x034DE2:
		lea     (Obj_Player_One).w, A1		       ; $FFFFB000
		cmpi.w  #$3990, x_pos(A1)				; $0010
		bcs.s   Offset_0x034DFC
		tst.b   objoff_2E(A1)				    ; $002E
		bne.s   Offset_0x034DFC
		btst    #$03, status(A1)		             ; $002A
		bne.s   Offset_0x034E04
Offset_0x034DFC:
		move.w  (Camera_X).w, (Sonic_Level_Limits_Min_X).w ; $FFFFEE78, $FFFFEE14
		rts
Offset_0x034E04:
		move.b  #$04, routine(A0)		            ; $0005
		bset    #$00, render_flags(A0)		              ; $0004
		move.w  #$001F, objoff_2E(A0)		            ; $002E
		move.l  #Offset_0x034E44, child(A0)		  ; $0034
		move.w  #$05C0, (Sonic_Level_Limits_Min_Y).w         ; $FFFFEE18
		lea     Knuckles_Palette(PC), A1               ; Offset_0x0355EE
		jmp     (Pal_Load_Line_1)		      ; Offset_0x04314C  
;-------------------------------------------------------------------------------
Offset_0x034E2E:
		subq.w  #$04, x_pos(A0)				  ; $0010
		lea     (Offset_0x0355B1), A1
		jsr     (Animate_Raw_A1)		       ; Offset_0x042092
		jmp     (Run_Object_Wait_Timer_A0)             ; Offset_0x0423D2   
;-------------------------------------------------------------------------------
Offset_0x034E44:
		move.b  #$06, routine(A0)		            ; $0005
		move.b  #$20, mapping_frame(A0)		             ; $0022
		move.w  #$001F, objoff_2E(A0)		            ; $002E
		move.l  #Offset_0x034E60, child(A0)		  ; $0034
		rts            
;-------------------------------------------------------------------------------
Offset_0x034E60:
		move.b  #$08, routine(A0)		            ; $0005
		bra     Offset_0x034D0C        
;-------------------------------------------------------------------------------
Offset_0x034E6A:
		tst.b   render_flags(A0)				    ; $0004
		bmi     Offset_0x034CF8
		lea     Knuckles_Palette_CNz_Lamp(PC), A1      ; Offset_0x03560E
		jsr     (Pal_Load_Line_1)		      ; Offset_0x04314C
		lea     Offset_0x034E92(PC), A2
		jsr     (SetupChildObject)		 ; Offset_0x041D9A
		jsr     (Remove_From_Tracking_Slot)            ; Offset_0x042BFA
		jmp     (Go_Delete_Object_A0)		  ; Offset_0x042D3E
;-------------------------------------------------------------------------------
Offset_0x034E92:
		dc.w    $0001
		dc.l    Inc_Level_Gradual_Min_Y		; Offset_0x0425C0
		dc.b    $00, $00
		dc.l    Inc_Level_Gradual_Max_X		; Offset_0x042564
		dc.b    $00, $00		    
;------------------------------------------------------------------------------- 
Knuckles_In_Carnival_Night_2_Boss:		             ; Offset_0x034EA0
		lea     Offset_0x034ED4(PC), A1
		jsr     (Check_Camera_In_Range)		; Offset_0x043392
		moveq   #$00, D0
		move.b  routine(A0), D0		              ; $0005
		move.w  Offset_0x034EC8(PC, D0), D1
		jsr     Offset_0x034EC8(PC, D1)
		lea     Knuckles_PLC_Data(PC), A2              ; Offset_0x0355A4
		jsr     (LoadDynamicPLC)		  ; Offset_0x042A0A
		jmp     (Check_Delete_Touch_Slotted)           ; Offset_0x042C1E  
;-------------------------------------------------------------------------------
Offset_0x034EC8:
		dc.w    Offset_0x034EDC-Offset_0x034EC8
		dc.w    Offset_0x034F48-Offset_0x034EC8
		dc.w    Offset_0x034F6A-Offset_0x034EC8
		dc.w    Offset_0x034F92-Offset_0x034EC8
		dc.w    Offset_0x034F6A-Offset_0x034EC8
		dc.w    Offset_0x035014-Offset_0x034EC8
;-------------------------------------------------------------------------------
Offset_0x034ED4:
		dc.w    $0176, $0300, $1B00, $1D00		
;-------------------------------------------------------------------------------
Offset_0x034EDC:
		lea     Knuckles_Setup_Data(PC), A1            ; Offset_0x035548
		jsr     (SetupSlottedObjectAttributes)              ; Offset_0x04298C
		move.l  #Offset_0x0355CB, child_data(A0)             ; $0030
		moveq   #Volume_Down, D0				  ; -$20
		jsr     (PlaySound)		           ; Offset_0x001176
		move.w  #$0078, objoff_2E(A0)		            ; $002E
		move.b  #$1F, angle(A0)		              ; $0026
		move.w  (Sonic_Level_Limits_Min_Y).w, (Target_Camera_Min_Y).w ; $FFFFEE18, $FFFFFA96
		move.w  (Level_Limits_Max_Y).w, (Target_Camera_Max_Y).w ; $FFFFEE12, $FFFFFA98
		move.w  (Sonic_Level_Limits_Min_X).w, (Target_Camera_Min_X).w ; $FFFFEE14, $FFFFFA94
		move.w  (Sonic_Level_Limits_Max_X).w, (Target_Camera_Max_X).w ; $FFFFEE16, $FFFFFA92
		move.w  #$0280, (Level_Limits_Max_Y).w               ; $FFFFEE12
		move.w  #$1D00, inertia(A0)		          ; $001C
		move.w  #$1D00, (Sonic_Level_Limits_Max_X).w         ; $FFFFEE16
		move.l  #Offset_0x034F54, child(A0)		  ; $0034
		lea     Knuckles_Palette(PC), A1               ; Offset_0x0355EE
		jsr     (Pal_Load_Line_1)		      ; Offset_0x04314C
		lea     Offset_0x03557E(PC), A2
		jmp     (SetupChildObject)		 ; Offset_0x041D9A  
;------------------------------------------------------------------------------- 
Offset_0x034F48:
		jsr     (AnimateRaw)		          ; Offset_0x04208E
		jmp     (Update_Sonic_Level_Limits_X_Y_Play_Music) ; Offset_0x0433DE   
;-------------------------------------------------------------------------------
Offset_0x034F54:
		move.b  #$04, routine(A0)		            ; $0005
		move.w  #$003F, objoff_2E(A0)		            ; $002E
		move.l  #Offset_0x034F76, child(A0)		  ; $0034
		rts         
;-------------------------------------------------------------------------------
Offset_0x034F6A:
		jsr     (AnimateRaw)		          ; Offset_0x04208E
		jmp     (Run_Object_Wait_Timer_A0)             ; Offset_0x0423D2  
;-------------------------------------------------------------------------------
Offset_0x034F76:
		move.b  #$06, routine(A0)		            ; $0005
		bset    #$00, render_flags(A0)		              ; $0004
		move.w  #$FA00, y_vel(A0)		          ; $001A
		move.w  #$0140, x_vel(A0)		          ; $0018
		bra     Offset_0x034D18           
;-------------------------------------------------------------------------------
Offset_0x034F92:
		jsr     (AnimateRaw)		          ; Offset_0x04208E
		jsr     (ObjectFall)		           ; Offset_0x0110FE
		tst.w   y_vel(A0)				  ; $001A
		bmi.s   Offset_0x034FCC
		jsr     (ObjHitFloor)		          ; Offset_0x009D84
		tst.w   D1
		bpl.s   Offset_0x034FCC
		moveq   #$00, D0
		move.b  objoff_39(A0), D0		       ; $0039
		cmpi.b  #$08, D0
		bcc.s   Offset_0x034FCE
		move.l  Offset_0x034FF0(PC, D0), x_vel(A0)         ; $0018
		addq.b  #$04, D0
		move.b  D0, objoff_39(A0)		       ; $0039
		bchg    #00, render_flags(A0)		               ; $0004
Offset_0x034FCC:
		rts
Offset_0x034FCE:
		move.b  #$08, routine(A0)		            ; $0005
		bclr    #$00, render_flags(A0)		              ; $0004
		add.w   D1, y_pos(A0)				    ; $0014
		move.w  #$003F, objoff_2E(A0)		            ; $002E
		move.l  #Offset_0x034FF8, child(A0)		  ; $0034
		bra     Offset_0x034CE2		  
;-------------------------------------------------------------------------------
Offset_0x034FF0:
		dc.w    $FF00, $FC00, $0100, $FC00      
;-------------------------------------------------------------------------------
Offset_0x034FF8:
		move.b  #$0A, routine(A0)		            ; $0005
		bset    #$00, render_flags(A0)		              ; $0004
		move.w  #$FA00, y_vel(A0)		          ; $001A
		move.w  #$0400, x_vel(A0)		          ; $0018
		bra     Offset_0x034D18  
;-------------------------------------------------------------------------------
Offset_0x035014:
		jsr     (AnimateRaw)		          ; Offset_0x04208E
		jsr     (ObjectFall)		           ; Offset_0x0110FE
		tst.b   render_flags(A0)				    ; $0004
		bmi     Offset_0x034CF8
		lea     Knuckles_Palette_CNz_Boss(PC), A1      ; Offset_0x03562E
		jsr     (Pal_Load_Line_1)		      ; Offset_0x04314C
		lea     Offset_0x03504E(PC), A2
		jsr     (SetupChildObject)		 ; Offset_0x041D9A
		move.w  (Target_Camera_Max_Y).w, (Level_Limits_Max_Y).w ; $FFFFFA98, $FFFFEE12
		jsr     (Remove_From_Tracking_Slot)            ; Offset_0x042BFA
		jmp     (Go_Delete_Object_A0)		  ; Offset_0x042D3E  
;-------------------------------------------------------------------------------
Offset_0x03504E:
		dc.w    $0002
		dc.l    Inc_Level_Gradual_Min_Y		; Offset_0x0425C0
		dc.b    $00, $00
		dc.l    Inc_Level_Gradual_Min_X		; Offset_0x042592
		dc.b    $00, $00
		dc.l    Inc_Level_Gradual_Max_X		; Offset_0x042564
		dc.b    $00, $00   
;-------------------------------------------------------------------------------
Offset_0x035062:
		move.w  parent3(A0), A1		            ; $0046
		btst    #$07, status(A1)		             ; $002A
		bne.s   Offset_0x035088
		move.b  #$08, width_pixels(A0)		              ; $0007
		moveq   #$13, D1
		move.w  #$0100, D2
		move.w  #$0200, D3
		move.w  x_pos(A0), D4				    ; $0010
		jmp     (Solid_Object_2)		       ; Offset_0x0135B6
Offset_0x035088:
		jmp     (DeleteObject)		         ; Offset_0x011138
;------------------------------------------------------------------------------- 
Knuckles_In_Launch_Base_1:				     ; Offset_0x03508E
		moveq   #$00, D0
		move.b  routine(A0), D0		              ; $0005
		move.w  Offset_0x0350AC(PC, D0), D1
		jsr     Offset_0x0350AC(PC, D1)
		lea     Knuckles_PLC_Data(PC), A2              ; Offset_0x0355A4
		jsr     (LoadDynamicPLC)		  ; Offset_0x042A0A
		jmp     (Check_Delete_Touch_Slotted)           ; Offset_0x042C1E   
;-------------------------------------------------------------------------------
Offset_0x0350AC:
		dc.w    Offset_0x0350BC-Offset_0x0350AC
		dc.w    Offset_0x0350E6-Offset_0x0350AC
		dc.w    CutKnux_Wait-Offset_0x0350AC
		dc.w    Offset_0x03512E-Offset_0x0350AC
		dc.w    CutKnux_Wait-Offset_0x0350AC
		dc.w    Offset_0x034F6A-Offset_0x0350AC
		dc.w    Offset_0x034F6A-Offset_0x0350AC
		dc.w    Offset_0x0351AC-Offset_0x0350AC     
;-------------------------------------------------------------------------------
Offset_0x0350BC:
		lea     Knuckles_Setup_Data(PC), A1            ; Offset_0x035548
		jsr     (SetupSlottedObjectAttributes)              ; Offset_0x04298C
		move.b  #$16, mapping_frame(A0)		             ; $0022
		move.w  #$00A0, (Sonic_Level_Limits_Min_Y).w         ; $FFFFEE18
		lea     Knuckles_Palette(PC), A1               ; Offset_0x0355EE
		jsr     (Pal_Load_Line_1)		      ; Offset_0x04314C
		lea     Offset_0x035586(PC), A2
		jsr     (SetupChildObject)		 ; Offset_0x041D9A   
;-------------------------------------------------------------------------------
Offset_0x0350E6:
		btst    #$03, objoff_38(A0)		     ; $0038
		bne.s   Offset_0x0350F0
		rts
Offset_0x0350F0:
		move.b  #$04, routine(A0)		            ; $0005
		moveq   #Volume_Down, D0				  ; -$20
		jsr     (PlaySound)		           ; Offset_0x001176
		move.w  #$0077, objoff_2E(A0)		            ; $002E
		move.l  #Offset_0x03510E, child(A0)		  ; $0034
		rts       
;-------------------------------------------------------------------------------
Offset_0x03510E:
		move.b  #$06, routine(A0)		            ; $0005
		moveq   #Knuckles_Theme_Snd, D0		            ; $1F
		jsr     (PlaySound)		           ; Offset_0x001176
		move.l  #Offset_0x0355CF, child_data(A0)             ; $0030
		move.l  #Offset_0x035134, child(A0)		  ; $0034
		rts  
;-------------------------------------------------------------------------------
Offset_0x03512E:
		jmp     (Animate_Raw_Multi_Delay)              ; Offset_0x04215C 
;-------------------------------------------------------------------------------
Offset_0x035134:
		move.b  #$08, routine(A0)		            ; $0005
		move.w  #$000F, objoff_2E(A0)		            ; $002E
		move.l  #Offset_0x03515E, child(A0)		  ; $0034
		lea     Offset_0x03558E(PC), A2
		jsr     (SetupChildObject)		 ; Offset_0x041D9A
		lea     (PLC_After_Knuckles_LBz_1), A1         ; Offset_0x041BBA
		jmp     (LoadPLC_Direct)		           ; Offset_0x001502  
;-------------------------------------------------------------------------------
Offset_0x03515E:
		move.b  #$0A, routine(A0)		            ; $0005
		move.w  #$007F, objoff_2E(A0)		            ; $002E
		move.l  #Offset_0x035176, child(A0)		  ; $0034
		bra     Offset_0x034CE2		  
;-------------------------------------------------------------------------------
Offset_0x035176:
		move.b  #$0C, routine(A0)		            ; $0005
		move.w  #$005F, objoff_2E(A0)		            ; $002E
		move.l  #Offset_0x035194, child(A0)		  ; $0034
		lea     Offset_0x035596(PC), A2
		jmp     (SetupChildObject_Simple)          ; Offset_0x041F5A  
;-------------------------------------------------------------------------------
Offset_0x035194:
		move.b  #$0E, routine(A0)		            ; $0005
		move.l  #Offset_0x0355B1, child_data(A0)             ; $0030
		clr.b   anim_frame_duration(A0)				 ; $0024
		clr.b   anim_frame(A0)				; $0023
		rts         
;-------------------------------------------------------------------------------
Offset_0x0351AC:
		tst.b   render_flags(A0)				    ; $0004
		bpl     Offset_0x0351BE
		addq.w  #$02, x_pos(A0)				  ; $0010
		jmp     (AnimateRaw)		          ; Offset_0x04208E
Offset_0x0351BE:
		clr.b   (Knuckles_Control_Lock_Flag).w               ; $FFFFFAA9
		jsr     (Restore_PlayerControl)               ; Offset_0x0432EE
		lea     (Obj_Player_Two).w, A1		       ; $FFFFB04A
		jsr     (Restore_PlayerControl2)            ; Offset_0x0432F2
		move.w  #$3B20, (Target_Camera_Max_X).w              ; $FFFFFA92
		lea     (Level_Resize_Max_X), A2               ; Offset_0x04261C
		jsr     (SetupChildObject_Simple)          ; Offset_0x041F5A
		move.w  #$0148, (Level_Limits_Max_Y).w               ; $FFFFEE12
		jsr     (Remove_From_Tracking_Slot)            ; Offset_0x042BFA
		lea     Knuckles_Palette_LBz(PC), A1           ; Offset_0x03564E
		jsr     (Pal_Load_Line_1)		      ; Offset_0x04314C
		jmp     (Go_Delete_Object_A0)		  ; Offset_0x042D3E  
;-------------------------------------------------------------------------------
Offset_0x035200:
		move.w  parent3(A0), A1		            ; $0046
		btst    #$07, status(A1)		             ; $002A
		bne.s   Offset_0x035238
		lea     Offset_0x035258(PC), A1
		jsr     (Check_Player_In_Range)		; Offset_0x043104
		tst.l   D0
		beq.s   Offset_0x035236
		tst.w   D0
		beq.s   Offset_0x03522C
		move.w  parent3(A0), A2		            ; $0046
		bset    #$03, objoff_38(A2)		     ; $0038
		bsr     Offset_0x03523E
Offset_0x03522C:
		swap.w  D0
		tst.w   D0
		beq.s   Offset_0x035236
		bsr     Offset_0x03523E
Offset_0x035236:
		rts
Offset_0x035238:
		jmp     (DeleteObject)		         ; Offset_0x011138
Offset_0x03523E:
		st      (Knuckles_Control_Lock_Flag).w               ; $FFFFFAA9
		move.w  D0, A1
		move.b  #$81, objoff_2E(A1)		              ; $002E
		bclr    #$00, render_flags(A1)		              ; $0004
		bclr    #$00, status(A1)		             ; $002A
		rts
;-------------------------------------------------------------------------------
Offset_0x035258:
		dc.w    $FFC0, $0080, $FFD0, $0060       
;-------------------------------------------------------------------------------
Offset_0x035260:
		lea     Offset_0x035566(PC), A1
		jsr     (SetupObjectAttributes)		      ; Offset_0x041D72
		move.l  #Offset_0x03527C, (A0)
		move.w  #$FE00, x_vel(A0)		          ; $0018
		move.w  #$FC00, y_vel(A0)		          ; $001A
Offset_0x03527C:		
		jsr     (Move_Light_Gravity)		   ; Offset_0x0426C2
		jmp     (Delete_Sprite_Check_X_Y)              ; Offset_0x042AD0   
;-------------------------------------------------------------------------------
Offset_0x035288:
		move.l  #Offset_0x0352C6, (A0)
		move.l  #Obj_Normal_Explode, child(A0) ; Offset_0x041C30, $0034
		moveq   #$00, D0
		move.b  subtype(A0), D0		              ; $002C
		add.w   D0, D0
		lea     Offset_0x0352B6(PC, D0), A1
		move.w  (A1)+, x_pos(A0)				 ; $0010
		move.w  (A1)+, y_pos(A0)				 ; $0014
		move.b  #$0A, subtype(A0)		            ; $002C
		jmp     (Offset_0x041C9A)
;-------------------------------------------------------------------------------
Offset_0x0352B6:
		dc.w    $3BC0, $01A0, $3B80, $01A0, $3B40, $01A0, $3B00, $01A0 
;-------------------------------------------------------------------------------
Offset_0x0352C6:
		subq.w  #$04, y_pos(A0)				  ; $0014
		jmp     (Run_Object_Wait_Timer_A0)             ; Offset_0x0423D2 
;------------------------------------------------------------------------------- 
Knuckles_In_Launch_Base_2:				     ; Offset_0x0352D0
		moveq   #$00, D0
		move.b  routine(A0), D0		              ; $0005
		move.w  Offset_0x0352F2(PC, D0), D1
		jsr     Offset_0x0352F2(PC, D1)
		bsr     Offset_0x035538
		lea     Knuckles_PLC_Data(PC), A2              ; Offset_0x0355A4
		jsr     (LoadDynamicPLC)		  ; Offset_0x042A0A
		jmp     (Check_Delete_Touch_Slotted)           ; Offset_0x042C1E     
;-------------------------------------------------------------------------------
Offset_0x0352F2:
		dc.w    Offset_0x0352FE-Offset_0x0352F2
		dc.w    Offset_0x03532E-Offset_0x0352F2
		dc.w    Offset_0x035348-Offset_0x0352F2
		dc.w    Offset_0x035362-Offset_0x0352F2
		dc.w    Offset_0x035384-Offset_0x0352F2
		dc.w    Offset_0x0353B2-Offset_0x0352F2    
;-------------------------------------------------------------------------------
Offset_0x0352FE:
		lea     Knuckles_Setup_Data(PC), A1            ; Offset_0x035548
		jsr     (SetupSlottedObjectAttributes)              ; Offset_0x04298C
		bset    #$00, render_flags(A0)		              ; $0004
		move.b  #$20, mapping_frame(A0)		             ; $0022
		move.w  #$0078, default_y_radius(A0)		         ; $0044
		lea     Knuckles_Palette(PC), A1               ; Offset_0x0355EE
		jsr     (Pal_Load_Line_1)		      ; Offset_0x04314C
		lea     Offset_0x03559C(PC), A2
		jmp     (SetupChildObject_Repeat)          ; Offset_0x041E4E  
;-------------------------------------------------------------------------------
Offset_0x03532E:
		btst    #$01, objoff_38(A0)		     ; $0038
		bne.s   Offset_0x035338
		rts
Offset_0x035338:
		move.b  #$04, routine(A0)		            ; $0005
		move.l  #Offset_0x035352, child(A0)		  ; $0034
		rts      
;-------------------------------------------------------------------------------
Offset_0x035348:
		lea     Offset_0x0355DE(PC), A1
		jmp     (Animate_Raw_Multi_Delay_A1)           ; Offset_0x042160    
;-------------------------------------------------------------------------------
Offset_0x035352:
		move.b  #$06, routine(A0)		            ; $0005
		bclr    #$00, render_flags(A0)		              ; $0004
		bra     Offset_0x034CE2           
;-------------------------------------------------------------------------------
Offset_0x035362:
		tst.b   (Boss_Attack_Started).w		      ; $FFFFFAA2
		beq.s   Offset_0x03537E
		move.b  #$08, routine(A0)		            ; $0005
		move.l  #Offset_0x0355AC, child_data(A0)             ; $0030
		clr.b   anim_frame_duration(A0)				 ; $0024
		clr.b   anim_frame(A0)				; $0023
Offset_0x03537E:
		jmp     (AnimateRaw)		          ; Offset_0x04208E    
;-------------------------------------------------------------------------------
Offset_0x035384:
		btst    #$03, objoff_38(A0)		     ; $0038
		bne.s   Offset_0x035392
		jmp     (AnimateRaw)		          ; Offset_0x04208E
Offset_0x035392:
		move.b  #$0A, routine(A0)		            ; $0005
		bset    #$00, render_flags(A0)		              ; $0004
		move.b  #$09, mapping_frame(A0)		             ; $0022
		move.w  #$0200, x_vel(A0)		          ; $0018
		move.w  #$FF00, y_vel(A0)		          ; $001A
		rts     
;-------------------------------------------------------------------------------
Offset_0x0353B2:
		jmp     (Move_Light_Gravity)		   ; Offset_0x0426C2 
;-------------------------------------------------------------------------------
Offset_0x0353B8:
		lea     Offset_0x035572(PC), A1
		jsr     (SetupObjectAttributes)		      ; Offset_0x041D72
		move.l  #Offset_0x035400, (A0)
		moveq   #$00, D0
		move.b  subtype(A0), D0		              ; $002C
		add.w   D0, D0
		move.l  Offset_0x0353E4(PC, D0), objoff_3E(A0)  ; $003E
		lsl.w   #$02, D0
		add.w   D0, y_pos(A0)				    ; $0014
		move.w  x_pos(A0), objoff_3A(A0)         ; $0010, $003A
		rts				     
;-------------------------------------------------------------------------------
Offset_0x0353E4:
		dc.w    $0200, $0020, $0180, $0018, $0100, $0010, $0080, $0008
		dc.w    $0040, $0004, $0000, $0000, $0000, $0000
;-------------------------------------------------------------------------------  
Offset_0x035400:
		tst.b   (Boss_Attack_Started).w		      ; $FFFFFAA2
		beq.s   Offset_0x035418
		move.l  #Offset_0x03541E, (A0)
		move.w  objoff_3E(A0), x_vel(A0)   ; $003E, $0018
		move.b  #$04, objoff_39(A0)		     ; $0039
Offset_0x035418:
		jmp     (Delete_Sprite_Check_X_Y)              ; Offset_0x042AD0
;-------------------------------------------------------------------------------
Offset_0x03541E:
		move.w  x_pos(A0), D0				    ; $0010
		move.w  objoff_40(A0), D1		       ; $0040
		cmp.w   objoff_3A(A0), D0		       ; $003A
		scs     D2
		bcs.s   Offset_0x035430
		neg.w   D1
Offset_0x035430:
		add.w   D1, x_vel(A0)		              ; $0018
		cmpi.b  #$0A, subtype(A0)		            ; $002C
		bcc.s   Offset_0x03545C
		cmp.b   objoff_3C(A0), D2		       ; $003C
		beq.s   Offset_0x03545C
		move.b  D2, objoff_3C(A0)		       ; $003C
		subq.b  #$01, objoff_39(A0)		     ; $0039
		bne.s   Offset_0x03545C
		move.l  #Offset_0x035478, (A0)
		move.w  parent3(A0), A1		            ; $0046
		bset    #$03, objoff_38(A1)		     ; $0038
Offset_0x03545C:
		jsr     (SpeedToPos)		           ; Offset_0x01111E
		tst.b   subtype(A0)				  ; $002C
		bne.s   Offset_0x035472
		move.w  parent3(A0), A1		            ; $0046
		move.w  x_pos(A0), x_pos(A1)		      ; $0010, $0010
Offset_0x035472:
		jmp     (Delete_Sprite_Check_X_Y)              ; Offset_0x042AD0  
;-------------------------------------------------------------------------------
Offset_0x035478:
		jsr     (Move_Light_Gravity)		   ; Offset_0x0426C2
		jmp     (Delete_Sprite_Check_X_Y)              ; Offset_0x042AD0  
;===============================================================================
; Objeto 0xC7 - Knuckles nas cenas de introdução
; <<<- 
;===============================================================================  